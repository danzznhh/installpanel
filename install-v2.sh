...
EOF
  ...
  # Setup allocation otomatis (multi-port)
  read -p "Masukkan IP address untuk allocation: " ip_address
  read -p "Masukkan IP alias (boleh kosong): " ip_alias
  read -p "Masukkan Port awal (contoh: 7000): " port_start
  read -p "Masukkan Port akhir (contoh: 7500): " port_end
...
EOF
  ...
  echo "🎉 Semua allocation berhasil ditambahkan ke node '$node_name'."
}
uninstall_panel() {
  echo -e "                                                       "
  echo -e "${BLUE}[+] =============================================== [+]${NC}"
